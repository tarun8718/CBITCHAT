  ...
  socket.on("joinRoom", ({ username, room }) => {
    ...
    Room.find({ room: user.room }).exec((err, data) => {
      ...
      const message = formatMessage(
        user.room,
        "chatBot",
        `Welcome to ${user.room} ${user.username}.`
      );
      const username = user.username;
      socket.emit("message", { message, username });
    });

    if (user.username.substr(-1) === "i") {
      const message = formatMessage(
        user.room,
        "chatBot",
        `A wild ${user.username} Appeared.`
      );
    ...
    } else if (user.username.substr(-1) === "a") {
      const message = formatMessage(
        user.room,
        "chatBot",
        `Welcome ${user.username}, We hope you brought pizza.`
      );
      socket.broadcast.to(user.room).emit("message", { message, username });
